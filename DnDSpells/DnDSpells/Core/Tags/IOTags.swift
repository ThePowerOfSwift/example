...
final class IOTags: UICollectionView {
    ...
    func prepareSpacingBeetwenCells() {
        ...
        for (_, item) in collection.enumerated() {
            ...
            if bounds.width >= currentWidth + newWidth + 8 {
                currentWidth += newWidth + 8
                result.append(item.description)
                widthCells.append(newWidth)
            } else if bounds.width == currentWidth + newWidth { // Если можно запихнуть тютелька в тютильку
                currentWidth = 0.0
                result.append(item.description)
                widthCells.append(newWidth)
            } else { // если не получается запихнуть
                var viewWidth = bounds.width - (currentWidth) - 1
...
extension IOTags: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: widthCells[indexPath.row], height: 38)
    }
